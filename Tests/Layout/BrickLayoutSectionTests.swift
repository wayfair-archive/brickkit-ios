...
class BrickLayoutSectionTests: XCTestCase {
    ...
    func testNoDataSource() {
        let section = createSection([], heights: [], edgeInsets: UIEdgeInsets(), inset: 0, sectionWidth: 320)
    ...
    func testThatContinueCalculatingReturnsRightValue() {
        let totalNumber = 20
        let section = createSection(Array<CGFloat>(repeating: 1, count: totalNumber), heights: Array<CGFloat>(repeating: 50, count: totalNumber), edgeInsets: UIEdgeInsets(), inset: 0, sectionWidth: 320, invalidate: false)
