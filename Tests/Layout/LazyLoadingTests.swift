...
class LazyLoadingTests: XCTestCase {
    ...
    func testThatAlignRowHeightsWorksProperly() {
        ...
        let attributes = brickView.collectionViewLayout.layoutAttributesForElements(in: CGRect(x: 0, y: 0, width: 320, height: 500))?.sorted(by: {($0.0.indexPath as NSIndexPath).item < ($0.1.indexPath as NSIndexPath).item})
