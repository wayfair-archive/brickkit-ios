...
class BaseBrickCellTests: XCTestCase {
    ...
    func testSetAccessibilityIdentifierFromInit() {
        brickView.registerBrickClass(DummyBrick.self)
        let dummyBrick = DummyBrick("BrickIdentifierThatIsDefaultAccessibilityIdentifier", accessibilityIdentifier: "AccessibilityIdentifierForDummyBrickCell")
        let section = BrickSection(bricks: [dummyBrick])
        brickView.setSection(section)
        brickView.layoutSubviews()
        let dummyCell = brickView.cellForItemAtIndexPath(NSIndexPath(forItem: 0, inSection: 1)) as? DummyBrickCell
